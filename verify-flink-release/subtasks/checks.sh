...
function compare_downloaded_source_with_repo_checkout() {
  ...
  comm -3 \
    ...
    <(find ${source_directory} -type f | sed "s|${source_directory}/||g" | sort) \
      | tee ${working_directory}/diff-artifacts-and-checkout.out
}
...
function compare_notice_with_pom_changes() {
  ...
  git --no-pager log ${base_git_tag}..${new_release_git_tag}~1 -p -- **/pom.xml > ${working_directory}/pom-diff.out
  git --no-pager log ${base_git_tag}..${new_release_git_tag}~1 -p -- **/NOTICE > ${working_directory}/notice-diff.out
  cd -
}
