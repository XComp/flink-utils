...
function clone_repo() {
  ...
  if [[ "$#" != 3 ]]; then
    echo "Usage: <working-directory> <flink-git-tag> <checkout-directory>"
    return 1
  fi
  ...
  git clone --depth 1 --branch release-${flink_git_tag} git@github.com:apache/flink.git ${checkout_directory} 2>&1 | tee ${working_directory}/git-clone.out
}
